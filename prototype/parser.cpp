...
vector<string> Parser::splitstr(string ori, char deli){
		vector<string> tmp; stringstream ss(ori); string
		tok; while(getline(ss, tok, deli)){
			tmp.push_back(tok);
		} return tmp;
	}
	...
	if(req.compare("sendLeader") == 0){
		...
		vector<User> tmp; for(int i = 2; i < params.size()/6;
		i += 6){
				User t;
			t.setIP(params[i]); t.setNickname(params[i+1]);
		...
		assert(params.size() == 5); cn->addUser(params[2],
		params[3], atoi(params[4].c_str()));

	}else if(req.compare("multicastUserlist") == 0){
		assert(params.size() == 2); cn->multicastUserlist();
	}
