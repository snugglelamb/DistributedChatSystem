...
std::string getlocalinfo()
{
	...
	if (debug) printf("local address: %s\n", ip);
	if (debug) printf("local port: %d\n", port);
	...
	if (debug) std::cout << "stub: local ip:port ->" << msg_ << std::endl;
    if (debug) printf("stub: waiting to recvfrom...\n");
...
std::string stub_create()
{
	...
	// port = 20000;
	sprintf(port_,"%d", port);
	if (debug) printf("stub: PORT specified: %s\n", port_);
...
std::string stub_send(const char* Tip, const char* Tport, const char* msg, int request)
{
	...
	if (sendQ_num < QUEUESIZE) { 
		if (debug) printf("stub: send msg enqueue.\n");
	...
	if (request < 1) {
		...
		if (clock_num == 0) {
			// no former target
			if (debug) printf("stub: clock new target.\n");
			...
			if (i == clock_num) {
				...
				if (debug) printf("stub: clock new target, num = 1\n");
				vclock[clock_num].label = key;
				vclock[clock_num].num = 1;
				clock_num++;
				valid_send_num = 1; 
				...
				if (debug) printf("stub: clock update target, num = %d.\n", valid_send_num);
		...
		if (debug) printf("SSSSTUB: get info: %s\n", info);
    ...
    if (debug) printf("stub: received packet from %s\n",
 ...
 //   printf("stub: ack received contains: %s\n", buf);
	...
	if ( strcmp(buf, "OK") == 0 ) {
		// dequeue
		if (debug) printf("stub: send msg dequeue.\n");
		sendQ_num--;
		sendID[sendQ_num] = sendQ[available_id].id;
		sendQ[available_id] = empty_send;
